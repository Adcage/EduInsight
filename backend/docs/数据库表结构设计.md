...
| class_id | INT | FOREIGN KEY, NULL | 班级ID(学生角色时使用) |
...
| course_id | INT | FOREIGN KEY, NULL | 关联课程ID |
| uploader_id | INT | FOREIGN KEY, NOT NULL | 上传者ID |
...
| parent_id | INT | FOREIGN KEY, NULL | 父分类ID(NULL表示顶级分类) |
...
| material_id | INT | FOREIGN KEY, NOT NULL | 资料ID |
...
| class_teacher_id | INT | FOREIGN KEY, NULL | 班主任ID(关联users表) |
...
| enrolled_time | DATETIME | DEFAULT CURRENT_TIMESTAMP | 选课时间 |
| status | TINYINT | DEFAULT 1 | 状态(1:正常, 0:退课) |
| create_time | DATETIME | DEFAULT CURRENT_TIMESTAMP | 创建时间 |
| update_time | DATETIME | ON UPDATE CURRENT_TIMESTAMP | 更新时间 |
...
- UNIQUE `uk_course_student` ON (course_id, student_id)
...
| course_id | INT | FOREIGN KEY, NOT NULL | 课程ID |
| class_id | INT | FOREIGN KEY, NULL | 班级ID(可选,用于指定班级考勤) |
...
| attendance_id | INT | FOREIGN KEY, NOT NULL | 考勤任务ID |
| student_id | INT | FOREIGN KEY, NOT NULL | 学生ID |
| status | VARCHAR(20) | NOT NULL | 出勤状态(present/late/absent/leave) |
| check_in_time | DATETIME | NULL | 签到时间 |
| check_in_method | VARCHAR(20) | NULL | 签到方式(qrcode/manual/face) |
...
| poll_id | INT | FOREIGN KEY, NOT NULL | 投票ID |
| student_id | INT | FOREIGN KEY, NOT NULL | 学生ID |
...
| course_id | INT | FOREIGN KEY, NOT NULL | 课程ID |
| user_id | INT | FOREIGN KEY, NOT NULL | 提问者ID |
...
| question_id | INT | FOREIGN KEY, NOT NULL | 问题ID |
| user_id | INT | FOREIGN KEY, NOT NULL | 回答者ID |
...
| material_id | INT | FOREIGN KEY, NOT NULL | 资料ID |
| original_category_id | INT | FOREIGN KEY, NULL | 原分类ID |
| suggested_category_id | INT | FOREIGN KEY, NOT NULL | 建议分类ID |
| confidence | DECIMAL(5,4) | NULL | 置信度(0-1) |
...
| user_id | INT | FOREIGN KEY, NOT NULL | 接收者ID |
...
users (用户表)
  ...
  ├── 1:N → materials (资料表)
  ├── 1:N → courses (课程表 - 教师)
  ├── N:M → courses (课程表 - 学生) via course_students
