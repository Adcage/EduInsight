<template>
  <a-modal v-model:open="visible" :title="isEdit ? '编辑用户' : '添加用户'" :width="600" @cancel="handleCancel"
           @ok="handleSubmit">
    <a-form ref="formRef" :label-col="{ span: 6 }" :model="formData" :rules="rules" :wrapper-col="{ span: 16 }">
      ...
      <a-form-item v-if="formData.role?.toLowerCase() === 'student'" label="班级ID" name="classId">
        <a-input-number v-model:value="formData.classId" :min="1" placeholder="请输入班级ID" style="width: 100%"/>
      </a-form-item>
...
import {userApiCreatePost, userApiIntUserIdPut} from '@/api/userController'
