...
class LoginModel(CamelCaseModel):
    """登录模型"""
    email: EmailStr = Field(..., description="用户邮箱")
    password: str = Field(..., min_length=6, description="密码")

class RegisterModel(CamelCaseModel):
    """注册模型"""
...
class TokenResponseModel(CamelCaseModel):
    """令牌响应模型"""
...
class RefreshTokenModel(CamelCaseModel):
    """刷新令牌模型"""
    refresh_token: str = Field(..., description="刷新令牌")

class ChangePasswordModel(CamelCaseModel):
    """修改密码模型"""
