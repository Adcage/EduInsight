...
class OrderItemCreateModel(CamelCaseModel):
    """订单项创建模型"""
    product_id: int = Field(..., description="产品ID")
    quantity: int = Field(..., gt=0, description="数量")

class OrderCreateModel(CamelCaseModel):
    """订单创建模型"""
...
class OrderUpdateModel(CamelCaseModel):
    """订单更新模型"""
    status: Optional[OrderStatusEnum] = Field(None, description="订单状态")
    shipping_address: Optional[str] = Field(None, description="收货地址")
    notes: Optional[str] = Field(None, description="订单备注")

class OrderItemResponseModel(CamelCaseModel):
    """订单项响应模型"""
    id: int
    product_id: int
    quantity: int
...
class OrderResponseModel(CamelCaseModel):
    """订单响应模型"""
    id: int
    user_id: int
...
class OrderListResponseModel(CamelCaseModel):
    """订单列表响应模型"""
    orders: List[OrderResponseModel]
    total: int

class OrderPathModel(CamelCaseModel):
    """订单路径参数模型"""
    order_id: int = Field(..., description="订单ID")

class OrderQueryModel(CamelCaseModel):
    """订单查询模型"""
    status: Optional[OrderStatusEnum] = Field(None, description="订单状态")
